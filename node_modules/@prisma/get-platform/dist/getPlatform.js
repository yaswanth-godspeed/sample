...
function parseDistro(osReleaseInput) {
  ...
  const distroInfo = (0, import_ts_pattern.match)({ id, idLike }).with(
    ...
    ({ idLike: idLike2 }) => idLike2.includes("centos") || idLike2.includes("fedora") || idLike2.includes("rhel"),
    ({ id: originalDistro }) => ({
      targetDistro: "rhel",
      familyDistro: "rhel",
      originalDistro
...
async function getSSLVersion(args) {
  const libsslSpecificPaths = (0, import_ts_pattern.match)(args).with({ familyDistro: "musl" }, () => {
  ...
  if (openSSLVersionLine) {
    ...
    debug(`The parsed openssl version is: ${openSSLVersion2}`);
    if (openSSLVersion2) {
