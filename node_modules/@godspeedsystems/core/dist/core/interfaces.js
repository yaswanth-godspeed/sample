...
class GSFunction extends Function {
    constructor(yaml, workflows, nativeFunctions, _fn, args, isSubWorkflow, fnScript) {
        ...
        if ((str.match(/<(.*?)%/) && str.includes('%>')) || str.match(/(^|\/):([^/]+)/)) {
            this.args_script = (0, utils_1.compileScript)(this.args);
        }
        ...
        if (this.yaml.metrics) {
            ...
            for (let metric of this.metrics) {
                ...
                switch (metric.type) {
                    case 'counter':
                        metric.obj = new metrics_1.default.Counter({
                            name: metric.name,
                            help: metric.help,
                        ...
                        break;
                    case 'gauge':
                        metric.obj = new metrics_1.default.Gauge({
                            name: metric.name,
                            help: metric.help,
                            labelNames: Object.keys(metric.labels || {})
                        });
                        break;
                    case 'histogram':
                        metric.obj = new metrics_1.default.Histogram({
                            name: metric.name,
                            help: metric.help,
                            labelNames: Object.keys(metric.labels || {})
                        });
                        break;
                    case 'summary':
                        metric.obj = new metrics_1.default.Summary({
                            name: metric.name,
                            help: metric.help,
                            labelNames: Object.keys(metric.labels || {})
                        });
            ...
            try {
                ...
                if (String(this.yaml.fn).startsWith('datasource.')) {
                    ...
                    if (headers) {
                        ...
                        let tempObj = {};
                        Object.keys(Object.assign(Object.assign({}, headers), args.headers)).map(key => {
